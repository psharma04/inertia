...
// Destination Protocol Compatibility Tests
//
// These tests verify byte-for-byte binary compatibility between the Swift
// Destination implementation and the Python RNS reference implementation.
//
// Fixtures are generated by:
//   python Tests/Scripts/generate_fixtures.py
//
// Hash algorithm (matches RNS Python source):
//
//   SINGLE destination:
//     full_name = "<app_name>.<aspect>.<identity_hash_hex>"
//     destination_hash = SHA-256(full_name.encode())[0:16]
//     name_hash        = SHA-256("<app_name>.<aspect>".encode())[0:10]
//
//   PLAIN destination (no identity):
//     full_name = "<app_name>.<aspect>"
//     destination_hash = SHA-256(full_name.encode())[0:16]
//     name_hash        = SHA-256(full_name.encode())[0:10]
//
// Tests FAIL until Destination is implemented in ReticulumCrypto.
